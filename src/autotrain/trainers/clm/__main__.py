...
def process_input_data(config):
    ...
    if config.trainer in ("dpo", "reward"):
        if (
            ...
            or config.text_column not in train_data.column_names
        ):
            train_data = train_data.rename_column(config.text_column, "chosen")
        if (
            config.rejected_text_column != "rejected"
            or config.rejected_text_column not in train_data.column_names
        ):
    ...
    if config.trainer == "dpo":
        if (
            ...
            or config.prompt_text_column not in train_data.column_names
        ):
    ...
    if config.valid_split is not None:
        ...
        if config.trainer in ("dpo", "reward"):
            if (
                config.text_column != "chosen"
                or config.text_column not in valid_data.column_names
            ):
        ...
        if (
            config.prompt_text_column != "prompt"
            or config.prompt_text_column not in valid_data.column_names
        ):
            if config.trainer == "dpo":
